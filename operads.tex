...
%Right actions are more commonly used in the literature of operads so we emphasize that perspective.
...
\begin{definition} [\cite{may72geometry}, \cite{boardman1973homotopy}] \label{def: e-infinity operad and prop}
	An operad is said to be $E_\infty$ if its underlying \mbox{$\mathrm{S}$-module} is $E_\infty$, and a prop $\mathcal P$ is said to be $E_\infty$ if either $U_1(\mathcal P)$ or $U_2(\mathcal P)$ is an \mbox{$E_\infty$ operad}.
\end{definition}
